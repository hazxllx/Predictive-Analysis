 ...
 * - Resolving patient links from PMS
 * - Listing patients (role-based access)
 * - Fetching specific patient records
 *
 * Performance optimizations:
 * - lean() on user serialization helper (already used upstream)
 * - Non-blocking audit for record views
 * - Early returns to avoid unnecessary PMS fetches
 */
...
/**
 * Resolve the current user's linked PMS patient record
 */
...
/**
 * List patients (role-based: patient sees self, admin sees all from PMS)
 */
...
/**
 * Fetch a single patient by ID (patients can only access their own)
 */
const getPatientById = async (req, res) => {
  try {
    ...
    // Audit: Log patient record access (admin only) — non-blocking
    if (req.user.role === "admin") {
      void auditAsync({
        user_id: req.user._id.toString(),
        action_type: AUDIT_ACTIONS.PATIENT_RECORD_VIEWED,
        details: `Admin viewed patient record: ${patient.patient_id}`,
